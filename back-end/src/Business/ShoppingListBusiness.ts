...
import {ICreateListDTO,IDeleteListDB,IDeleteListDTO,OShoppingListDTO,ShoppingList,} from "../models/ShoppingList";
...
export class ShoppingListBusiness {
  ...
  createListBusiness = async (input: ICreateListDTO) => {
    ...
    for (const shoppingList of  list) {
      if (shoppingList.id_product === productId && shoppingList.order_id === null) {
        const newQuantity = shoppingList.quantity + 1;
        const response = await this.shoppingListData.updateQtyList(newQuantity,shoppingList.id);
        return response
      }
    } 
    ...
    for (const shoppingList of list) {
      if (shoppingList.id === id) {
        if (shoppingList.quantity > 1 && shoppingList.order_id === null) {
          ...
          const response = await this.shoppingListData.updateQtyList(newQuantity,shoppingList.id);
          return response;
        } else if (shoppingList.quantity === 1 && shoppingList.order_id === null) {
